            ...
            <div key={patient.id} className="surface-panel p-4">
              <div className="flex items-start justify-between">
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  <button
                    onClick={() => {
                      let numeroLimpo = patient.contato_whatsapp?.replace(/\D/g, '');
                      if (numeroLimpo) {
                        if (!numeroLimpo.startsWith('55')) numeroLimpo = `55${numeroLimpo}`;
                        window.open(`https://wa.me/${numeroLimpo}`, '_blank');
                      } else {
                        toast.error('Paciente sem número de WhatsApp cadastrado.');
                      }
                    }}
                    className="icon-button h-10 w-10 text-emerald-600 dark:text-emerald-400"
